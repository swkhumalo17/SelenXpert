...
@KeywordAnnotation(Keyword = "SubmitClaimsForApproval", createNewBrowserInstance = false)
public class SubmitClaimsForApproval extends BaseClass {
    String error = "";
    ...
    public SubmitClaimsForApproval() {
        this.approveClaimsObj = new Approve_Claims();
    }

    public TestResult executeTest() {
        if (!submitClaimsForApproval()) {
    ...
    public boolean submitClaimsForApproval() {
        approveClaimsObj.navigateToSTClaims();

        //wait for data to appear
        if (!SeleniumDriverInstance.waitForElementByXpath(DAE_PageObject.weekClaimsContent())) {
            error = "Failed to wait for the waiting week claim content.";
            return false;
        }
        ...
        pause(2000);
        if(!SeleniumDriverInstance.scrollToElement(DAE_PageObject.EndOdometer_txt())){
            error  = "Failed to scroll to the Submit Claim For Approval button.";
            return false;
        }
        ...
        if (!totalClaimAmount_popup.isEmpty()) {
            narrator.ValidateEqual(totalClaimAmount_popup, "R " + formattedTotalAmount);
            narrator.stepPassedWithScreenShot("Successfully validate the total claim amount '" + totalClaimAmount_popup + "' with 'R " + formattedTotalAmount + "'.");
        } else {
